...
- 可以監聽數組內部資料的變化，不需要對數組進行特殊處理，使得 Vue3.x 在處理數組時更加靈活。
...
- 能夠更好地處理動態組件的新增、刪除和移動，降低 Virtual DOM 的重新渲染次數。
...
- Vue3.x 靜態提升：靜態組件在編譯時就被編譯器轉換為常數，不需要在運行時再進行處理。
...
Vue 3.x 的 **Teleport** 功能可以讓你將一個組件的內容「傳送」（teleport）到 DOM 樹中的任何位置，而不需要將該組件包裝在其他組件中。這樣可以讓你更方便地控制組件的位置和渲染效果，不需要為了實現這些效果而添加額外的 DOM 結構或使用複雜的 CSS。
